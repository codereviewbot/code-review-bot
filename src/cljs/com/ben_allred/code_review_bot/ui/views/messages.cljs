    ...
    (let [add-button [:i.fa.fa-plus-circle.button.blue
        ...
        [:li.message-pair
         ...
         (if (empty? message-set)
             "no messages."
             [:ul.message-list
              (for [message-str (sort message-set)]
                  ^{:key (log/spy message-str)}
...
(defn ^:private compare-keys [key-1 key-2]
    (cond
         (= :EDIT-ME key-1) 1
         (= :EDIT-ME key-2) -1
...
(defn display [repo-id messages]
    [:div.messages
     [:h3 "Messages:"]
     [:ul.message-pairs
      (for [[message-key message-set] (sort-by key compare-keys messages)]
          ^{:key (str message-key)}
